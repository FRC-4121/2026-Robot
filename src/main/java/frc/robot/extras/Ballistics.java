...
public class Ballistics {
  ...
  public int angleIncrement = 1;// make sure this is a factor of the difference between min and max angle,
                                // unless you are only using one angle, then make it 1

  public int minSpeed = 20;// percent
  public int maxSpeed = 100;// percent
  public int argumentCount = 5;// distance, angle, speed, height, possibility(is the shot possible) (0 is
                               // false)

  /*
   * 2-d ballistics array
   * Structure: each internal array is [distance, angle, speed, height,
   * possibility of shot]
   */
   ...
   * 
   * @param launchH: The height, in inches, of the launcher release point above
   * the ground
   * 
   * @param tolerance: A additive factor, in inches, that widens the range to
   * target around the target height
   * 
   ...
   * 
   * @param slip: The percent of wheel speed that is converted to linear speed of
   * the missile (due to the single-wheel design, much is lost as rotational
   * speed)
   */
  public Ballistics(double targetH, double launchH, double tolerance, double maxRPM, double wheelD, double slip) {
    ...
    missileMaxSpeed = maxSpeedRPM * wheelCircumference * wheelSlip / 60;// in/s //the 60 is to convert RPM to RPS. wheel
                                                                        // circumference converts RPS to in/s
  ...
  public double[][] generateBallisticsTable() {
    ...
    double[][] table = new double[(maxDistance - minDistance) / distanceIncrement + 1][argumentCount]; // this makes a
                                                                                                       // matrix of
                                                                                                       // [distance,
                                                                                                       // angle, speed,
                                                                                                       // height,
    ...
    for (int i = 0; i < table.length; i++) { // length is number of rows. which is different distances
      ...
      for (int a = minAngle; a <= maxAngle; a += angleIncrement) {
        ...
        for (int s = minSpeed; s <= maxSpeed; s++) {
          ...
          // calculate the height that the ball will be at when it hits the wall and error
          // from target
          ...
          // If the error is the smallest yet, assign values of this configuration to
          // place in the table
          if (error < minError) {
  ...
  // Input distance in feet, angle in degrees, speed in percent; output height(the
  // ball will land at distance away) in inches
  public double calculateHeight(double distance, double angle, double speed) {
    // this equation looks like the one from
    // @Link:https://byjus.com/trajectory-formula/#:~:text=Trajectory%20formula%20is%20given%20by%20y%20%3D%20xtan%CE%B8%E2%88%92,2%20%CE%B8%20Where%2C%20y%20is%20the%20horizontal%20component%2C

    // Derived from parametric equations of t based on basic trajectories. Does not
    // account for air resistance; this should be accounted for in the 'slip factor'
    double height = tan(toRadians(angle)) * distance
        - 0.5 * g * pow(distance / (cos(toRadians(angle)) * missileMaxSpeed * speed), 2);
    // The equation^ is
    // y = vt -g/2t^2
  ...
  public double calcHorizDistToGoal(double DegreesAngle) {
    ...
    timeInAir[0] = (0 - horizVelocity)
        + Math.sqrt(Math.pow(horizVelocity, 2) - (4 * ((0 - g) / 2) * (0 - heightTolerance)));
    // timeInAir[1] = (0-horizVelocity) - Math.sqrt(Math.pow(horizVelocity, 2) - (4*
    // ((0-g)/2) * (0-targetHeight)));
  ...
  public double[] queryBallisticsTable(double distance) {
    ...
    // Default values
    double shotPossible = 0;// essentially a boolean that I can put into an array of doubles; 0 = false, 1 =
                            // true
    double angle = 0;
    double speed = 0;
    double dist = 0;
    ...
    // If outside the limits of the mechanism, the shot is not possible and we
    // return immediately
    if (distance < (double) minDistance || distance > (double) maxDistance) {
      ...
      // Otherwise, compare the entered distance to the incremented distances in the
      // table and grab data
    } else {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      // String address =
      // "C:/Users/113281/Documents/GitHub/2022-Robot-V2/src/main/java/frc/robot/ExtraClasses"
      // + "/outputTable.txt";//you can replace the address...also replace all \ with
      // /
      String address = "C:/Users/team4/OneDrive/Documents/GitHub/2022-Robot-V2/src/main/java/frc/robot/ExtraClasses"
          + "/outputTable.txt";// you can replace the address...also replace all \ with /
    ...
    while (scIn.next().toUpperCase().equals("Y")) {
      ...
      if (tableQuery[0] == 1) {
        ...
        // System.out.println(" [this is inaccurate information cuz didn't work
        // ->]Horizontal distance to goal " + ballistics.calcHorizDistToGoal(
        // 60/*degrees*/) + " inches.");
      } else {
